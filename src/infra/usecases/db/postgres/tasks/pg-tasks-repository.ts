...
export class PgTasksRepository implements IGetAllTasksRepository, IGetTasksByCategoryRepository {
  ...
  async getByCategory(model: GetTasksByCategoryModel): Promise<TaskByCategory[]> {
    const result = await PgHelper.query('SELECT tasks.id, tasks.name, tasks.text, task_by_category.user_id AS user_id, categories.name AS categoria, categories.id AS category_id  FROM tasks INNER JOIN task_by_category ON tasks.id = task_by_category.task_id INNER JOIN categories ON task_by_category.category_id = categories.id WHERE task_by_category.user_id = $1 AND categories.id = $2', [model.userId, model.categoryId])
    const foundTasks: TaskByCategory[] = []
    for (const task of result.rows) {
      const categorias: ICategory[] = []
      for (const pos of result.rows) {
        if (pos.categoryId === task.categoryId && pos.user_id === model.userId && pos.id === task.id) {
          categorias.push({
            id: pos.categoryId,
            name: pos.categoria
          })
